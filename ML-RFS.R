##############################################################################
# REVISED CODE WITH BEST MODEL SELECTION
##############################################################################
...
# NEW: We also store each RSF model and each test index vector for retrieval
rfs_models        <- list()
...
for (fold_name in names(multi_folds)) {
  ...
  rfs_models[[fold_counter]]        <- rfs_model
...
# so you have "rsf_models", "test_indices_list", etc.
...
for (t in seq_along(time_horizons)) {
  ...
  cal_list[[t]] <- vector("list", length(rfs_models))
  
  # For each fold
  for (f in seq_along(rfs_models)) {
    ...
    cal_result <- calPlot(
      object    = rfs_models[[f]],  # Ensure correct variable name
      time      = time_horizon,
      formula   = Hist(time, delta) ~ 1,
      data      = test_data,
      cause     = 1,
...
combined_plot <- ggplot(cal_summary, aes(x = PredRisk_Mean, y = ObsRisk_Mean, color = Time_Horizon)) +
  ...
  # Uncomment the following lines if you wish to include horizontal error bars
  # geom_errorbarh(
